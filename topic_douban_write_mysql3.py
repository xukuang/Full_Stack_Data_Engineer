...
db = pymysql.connect(host='127.0.0.1', user='root', passwd='iae', db='study', port=3306, charset='utf8')
db.autocommit(True)
cursor = db.cursor()
...
fw = open('data/douban_movie_clean_kuang_python3.txt', 'w', encoding = 'utf-8')
for i in movies:
	for j in i:
		fw.write(str(j) + '^')
	fw.write('\n')
